...
const CustomizationPage = () => {
  ...
  const handleAddText = () => {
    setTexts((prevTexts) => [
      ...prevTexts,
      { content: 'Your Text', x: 50, y: 50, color: '#000000', size: 12 },
    ]);
  };
  ...
  const handleSaveCustomization = () => {
    const customization = {
      ...
      styles: { ...styles },
      players,
      cartItems: players.length > 0 ? players.length : 1,
      totalPrice: players.length > 0 ? players.length * state.totalPrice : state.totalPrice,
    };
      ...
      <div className="grid grid-cols-12 gap-4 container mx-auto items-center flex-wrap pt-4 pb-12">
        ...
        <div className="col-span-12 md:col-span-6">
          <div className="flex-1">
            ...
            {!RenderShirt && <div>Error: Shirt type not found</div>}
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4 container mx-auto items-center justify-center flex-wrap pt-4 pb-12">
        <div className='flex w-1/4'>
          <button onClick={handleSaveCustomization}
            className='flex items-center justify-center text-white text-sm bg-green-500 rounded-sm px-2 py-2 w-full'
          >Add to cart</button>
        </div>
      </div>
    </div>
